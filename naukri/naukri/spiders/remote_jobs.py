...
class RemoteJobsSpider(scrapy.Spider):
    ...
    start_urls = ['https://www.naukri.com/jobapi/v3/search?\
                noOfResults=20\
    &urlType=search_by_keyword&searchType=adv&keyword=remote&pageNo=1\
    &seoKey=remote-jobs&src=discovery_trendingWdgt_homepage_srch&latLong=']

    page = 1
    def parse(self, response):
        payload = json.loads(response.body)
        total_page = payload['noOfJobs']
        page_count = ceil(total_page/20)
        for j in payload["jobDetails"]:
            ...
            while self.page <= page_count:
                ...
                base_url = 'https://www.naukri.com/jobapi/v3/search?\
                            noOfResults=20\
                            &urlType=search_by_keyword&searchType=adv&keyword=remote&pageNo={}\
